...
<div class="d-flex container flex-column content user-content p-0">
    <!-- Review Order Section -->
    <div class="track-order d-flex flex-column border-bottom flex-grow-1 bg-light text-black rounded shadow-sm mb-4">
        <!-- Header -->
        <div class="header-more p-3 border-bottom d-flex justify-content-between align-items-center">
            <h5 class="m-0 text-secondary">Order Completed</h5>
        </div>
        ...
        <div class="track-order-body px-4 py-3">
            @forelse ($completedOrders as $order)
                <!-- Top Section -->
                <div class="top fw-bold pb-3">
                    Your order has been delivered on {{ $order->delivered_at->format('M d, Y') }}.
                </div>

                <!-- Timeline -->
                <div class="timeline">
                    @foreach ($order->items as $item) <!-- Assuming `items` is a relationship for order items -->
                        <!-- Delivered Section -->
                        <div class="timeline-item d-flex align-items-start py-3 top">
                            <div class="order-width d-flex">
                                <div class="">
                                    <img src="{{ $item->menu->image }}" alt="{{ $item->menu->name }}" class=""
                                        style="max-height: 100px; width: auto;">
                                </div>
                                <div class="content ms-3">
                                    <div class="status fs-5">{{ $item->menu->name }}</div>
                                    <div class="location text-muted small">{{ $item->menu->description }}</div>
                                    <div class="location fs-5 flex-between">
                    ...
                    <div class="timeline-item py-3 d-flex align-items-start top">
                        <div class="content order-width d-flex flex-column gap-2">
                            ...
                            <div class="details text-muted flex-between">
                                <div>Delivery date</div>
                                <div>{{ $order->delivered_at->format('M d, Y h:i A') }}</div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="timeline-item py-3 d-flex align-items-start">
                        <div class="content order-width d-flex gap-2">
                            <a href="{{ route('user.buyAgain', $order->id) }}" class="btn btn-outline-dark w-50">Buy Again</a>
                            <a href="{{ route('user.viewReview', $order->id) }}" class="btn btn-outline-dark w-50">View Review</a>
                        </div>
                    </div>
                </div>
            @empty
